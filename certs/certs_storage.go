...
func (s *CertificatesStorage) CreateArchiveFolder() {
	err := common.CreateNonExistingFolder(s.archivePath)
	if err != nil {
		log.Fatalf("Could not check/create path: %v", err)
	}
}
...
func (s *CertificatesStorage) GetFileName(domain, extension string) string {
	...
	return filepath.Join(s.rootPath, baseFileName, filename)
}
...
func (s *CertificatesStorage) WriteFile(domain, extension string, data []byte) error {
	var baseFileName = sanitizedDomain(domain)
	err := common.CreateNonExistingFolder(filepath.Join(s.rootPath, baseFileName))
	if err != nil {
		log.Fatalf("Could not check/create path: %v", err)
	}
	filePath := filepath.Join(s.rootPath, baseFileName, baseFileName+extension)
